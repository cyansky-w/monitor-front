...
export async function getOptionTrend(option:object ,api:Function){
  ...
  // for(let item of xAxis){
    // newUser.push(res.data.newUserNumber[newIndex]?.timeType === item ? res.data.newUserNumber[newIndex++].dataNumber : 0)
    // newUser.push(Math.random()*1000)
    // oldUser.push(res.data.oldUserNumber[oldIndex]?.timeType === item ? res.data.oldUserNumber[oldIndex++].dataNumber : 0)
  // }
  
  option.xAxis.data=xAxis.map(value => {newUser.push(Math.random()*1000);oldUser.push(Math.random()*1000);return value});
  option.series[0].data= newUser;
  option.series[1].data= oldUser;
}
...
export async function getOptionStatistical(option:object ,api:Function){
  ...
  option.xAxis.data=xAxis.map(value => {data.push(Math.random()*1000);return value.substring(5)});
  option.series[0]={
    name: '新用户',
    type: 'line',
    stack: 'total',
